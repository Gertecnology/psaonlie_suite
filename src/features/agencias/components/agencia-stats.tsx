...
export function AgenciaStats({ agencia }: AgenciaStatsProps) {
  ...
  const getVentaHabilitadaColor = (ventaHabilitada?: string | null) => {
  ...
  const totalBoletosDisponibles =
    agencia.hijas?.reduce((total, hija) => total + (hija.boletosDisponibles ?? 0), 0) || 0
  const agenciasActivas = agencia.hijas?.filter(hija => hija.activo).length || 0
      ...
      <Card>
        ...
        <CardContent>
          <div className="text-2xl font-bold">
            <Badge
              variant="secondary"
              className={`${getStatusColor(agencia.activo)} border-0`}
            >
      ...
      <Card>
        ...
        <CardContent>
          <div className="text-2xl font-bold">
            <Badge
              variant="secondary"
              className={`${getVentaHabilitadaColor(agencia.ventaHabilitada)} border-0`}
            >
              {agencia.ventaHabilitada === "S" ? 'Habilitada' : 'Deshabilitada'}
            </Badge>
          </div>
        </CardContent>
      </Card>
      ...
      <Card>
        ...
        <CardContent>
          ...
          <p className="text-xs text-muted-foreground">
            {agencia.hijas ? `${agenciasActivas} de ${agencia.hijas.length} activas` : 'No hay agencias'}
          </p>
        </CardContent>
      </Card>
